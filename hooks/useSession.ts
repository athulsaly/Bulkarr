...
function localKey(target: Target) { return `bulkarr:session:${target}` }
...
export function useSession(serverSession: Session | null | undefined, target: Target): SessionState & SessionActions {
  const [state, setState] = useState<SessionState>(() => {
    const local = loadLocal(target)
    return {
      target,
      defaults: { ...DEFAULT_DEFAULTS, ...(serverSession?.defaults ?? {}), ...(local.defaults ?? {}) },
  ...
  const persist = useCallback((s: SessionState) => {
    localStorage.setItem(localKey(s.target), JSON.stringify({ defaults: s.defaults, rawInput: s.rawInput }))
    if (timerRef.current) clearTimeout(timerRef.current)
    timerRef.current = setTimeout(() => {
      const session: Session = { target: s.target, defaults: s.defaults, rawInput: s.rawInput, rows: s.rows, updatedAt: Date.now() }
      fetch('/api/settings', {
        method: 'POST',
  ...
  const setDefaults = useCallback((d: Partial<DefaultsConfig>) =>
    setState(prev => {
      const next = { ...prev, defaults: { ...prev.defaults, ...d } }
      persist(next)
      return next
    }), [persist])
  const setRawInput = useCallback((s: string) => update({ rawInput: s }), [update])
  const setRows = useCallback((rows: ReviewRow[]) => update({ rows }), [update])
  const updateRow = useCallback((id: string, patch: Partial<ReviewRow>) =>
    setState(prev => {
      const next = { ...prev, rows: prev.rows.map(r => r.id === id ? { ...r, ...patch } : r) }
      persist(next)
      return next
    }), [persist])
  ...
  return { ...state, setTarget, setDefaults, setRawInput, setRows, updateRow, clearSession }
}
