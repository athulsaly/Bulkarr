...
export default function AddSeriesPage() {
  ...
  const session = useSession(null, 'series')
  ...
  const handleLookup = useCallback(async () => {
    clearSummary()
    const rows = await lookup(session.rawInput, 'series', cache)
  ...
  const handleSubmit = useCallback(async () => {
    const s = await submit(session.rows, 'series', session.defaults, session.updateRow)
    addToast(
  ...
  const noMatchEntries = session.rows
    ...
    .map(row => ({ row, target: 'series' as const }))
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#2a2a3a] shrink-0">
        <h1 className="text-lg font-semibold text-slate-100">Add Series</h1>
